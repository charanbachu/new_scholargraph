...
class College_model extends CI_Model {
	...
	*/
	public function getAttributeChildNodes($node = 0,$nodeid,$collegeId)
	{
		...
		for($i=$attr_pos[$node];$i<sizeof($attr);$i++)
		{
			...
			if($attr[$i]['Prev_Node'] == $node && $tb2_attr_pos[$node][$attr[$i]['Node_ID']] != NULL)
			{
				...
				if($data[$j]['NODE_VALUE'] != NULL && $data[$j]['S_Node'] == $nodeid && $data[$j]['P_Node'] == $DNodes)
				{
					...
					echo 'If condition  1 Check => Node-id Internal: =>'.$nodeid.' Attribute:'.$attr[$i]['Node_ID'].'=>'.$time_elapsed_secs.'<br>';
					...
					{
						$child->footer_value = $this->getFooterValue($data[$j]['NODE_NAME'],$child->value);
					}
					else
					{
					...
					$time_elapsed_secs = microtime(true) - $start;
					echo 'Node-id Internal: =>'.$nodeid.' Attribute:'.$attr[$i]['Node_ID'].'=>'.$time_elapsed_secs.'<br>';
				}
				else if($data[$j]['S_Node'] == $nodeid && $data[$j]['P_Node'] == $DNodes)
				{
					$time_elapsed_secs = microtime(true) - $start;
					echo 'If condition  2 Check => Node-id Internal: =>'.$nodeid.' Attribute:'.$attr[$i]['Node_ID'].'=>'.$time_elapsed_secs.'<br>';
					$child = $this->buildAttributeTreeWithNodeId($collegeId,$nodeid,$data[$j]['Answer_Node'],0);
				...
				if($data[$j]['NODE_VALUE'] != NULL && $data[$j]['D_Node'] == $sNodes[$i])
				{
					if($public == 1)
					{
						if($this->isPublicNode($sNodes[$i]))
						{
							...
							{
								$child->footer_value = $this->getFooterValue($data[$j]['NODE_NAME'],$child->label);
							}
							else
							{
								$child->footer_value = $data[$j]['Footer_value'];
					...
					else
					{
						...
						if($data[$j]['footer_value_flag']==1)
						{
							$child->footer_value = $this->getFooterValue($data[$j]['NODE_NAME'],$data[$j]['NODE_VALUE']);
						}
						else
						{
							$child->footer_value = $data[$j]['Footer_value'];
	...
	*/
	...
	public function getFooterValue($node_name,$val)
	{
		/*
		Currently Available
		Streams/Schools
